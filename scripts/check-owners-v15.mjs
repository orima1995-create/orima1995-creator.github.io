...
try{for(const width of [320,390,768,1280]){
 ...
 assert(cards.every(c=>c.width>=120&&c.scroll<=c.width+1));if(width<=760)assert(cards.every(c=>c.height<=170));
 if(screens)await page.screenshot({path:path.join(screens,'owners-v15-directory-'+width+'.png')});
 const hrefs=await page.locator('.owner-frame').evaluateAll(els=>els.map(e=>e.href.split('#')[0]));
 // Stress the repeatable grid without adding fictitious entries to shipped data.
 ...
 for(const href of hrefs){
  ...
  if(screens)await page.screenshot({path:path.join(screens,new URL(href).pathname.includes('pierce')?'owners-v15-pierce-'+width+'.png':'owners-v15-cyma-'+width+'.png')});
