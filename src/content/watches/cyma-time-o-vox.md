...
deepDive:
  ...
  - number: "02"
    ...
    paragraphs:
      ...
      - 2つのプッシャーは互いに連動し、一方を押すともう一方が押し出される。上側を押し込むと巻上げ、中央位置ではアラームの作動待機、下側を押し込むとアラーム時刻設定となる。時刻合わせはリューズを引いて行う。
      - この切り替えを担うのが、Wippe（揺動レバー）と呼ばれる機構。内部には上下に重なる2枚のレバーがあり、プッシャーやリューズの操作に応じて、リューズの力を巻上げ・時刻合わせ・アラーム時刻設定のそれぞれへ振り分ける。一つのリューズを、用途ごとに使い分けるための仕組みだ。
    citationRefs:
      - "1"
      - "1"
      - "1"
    images:
      ...
      - src: /images/cyma-time-o-vox/wipe.jpg
        caption: |-
          Wippe（揺動切替機構）と上下プッシャーの位置関係。
          プッシャー操作に応じて、内部の輪列のつながりが切り替わる。
  ...
  - number: "03"
    ...
    paragraphs:
      - R.464でもう一つ大きな特徴が、時刻表示とアラームを一つの香箱から駆動すること。香箱とは、時計を動かすゼンマイを収める部分。
      - 時計用とアラーム用に別々の香箱を持つアラーム腕時計も多いが、R.464は一つの動力源を共有する。
      - アラームを鳴らせば、そのぶん時計を動かすためのエネルギーも減る。そこでR.464には、アラームの鳴動時間を制限する機構が組み込まれている。資料では、鳴動時間はおよそ6〜10秒。
      - 掲載個体では、鳴動時間を制御するタイミングホイールが約6秒間作動する様子を実機動画で確認できる。静止画では分かりにくい動きなので、Xに動画を掲載している。
  ...
  - number: "04"
    ...
    paragraphs:
      - 1香箱式のアラームには、鳴動にともなってリューズまで回転する構造もある。
      - R.464では、アラームが作動すると巻上げ中間車が巻上げ側から切り離される。そのため、アラームが鳴っている間もリューズは回転しない。
    citationRefs:
      - "3"
      - "1,3"
  - number: "05"
