...
deepDive:
  ...
  - number: "02"
    ...
    paragraphs:
      - |-
        ...
        この2つは、単なるアラームのON / OFFボタンではない。
      - |-
        2つのプッシャーは互いに連動し、一方を押すともう一方が押し出される。
        上・中央・下の3つの位置によって、巻上げ、時刻合わせ、アラーム時刻設定、鳴動の待機を切り替える。
        R.464では、一つのリューズで複数の操作を行うために、プッシャーで内部の接続先を切り替えている。
      - |-
  ...
  - number: "03"
    ...
    paragraphs:
      - |-
        R.464でもう一つ大きな特徴が、時刻表示とアラームを一つの香箱から駆動すること。
        香箱とは、時計を動かすゼンマイを収める部分。
      - 時計用とアラーム用に別々の香箱を持つアラーム腕時計も多いが、R.464は一つの動力源を共有する。
      - |-
        アラームを鳴らせば、そのぶん時計を動かすためのエネルギーも減る。
        そこでR.464には、アラームの鳴動時間を制限する機構が組み込まれている。
        資料では、鳴動時間はおよそ6〜10秒。
      - |-
        掲載個体では、鳴動時間を制御するタイミングホイールが作動する約6秒の様子を実機動画で確認できる。
